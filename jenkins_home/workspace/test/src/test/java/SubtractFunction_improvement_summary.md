...
Here are five test suites for the `SubtractFunction` class using JUnit:
...
Note that the `testDisplayResult` method doesn't assert the result of the `displayResult` method directly, because it doesn't return anything. However, we can verify that it doesn't throw any exception.